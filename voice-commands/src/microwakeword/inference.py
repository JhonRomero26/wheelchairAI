...
class Model:
    ...
    Args:tflm_output_quant -output_zero_point) and
    ...
    def predict_spectrogram(self, spectrogram: np.ndarray):
        ...
        for chunk in chunks:
            ...
            for output_detail in self.output_details:
                output_data = self.model.get_tensor(output_detail["index"])[0]
                if self.is_quantized_model:
                    output_data = self.dequantize_output_data(
                        output_data, output_detail
                    )
